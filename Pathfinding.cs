...
public class Pathfinding
{
    ...
    public List<(int, List<(float, float)>)> pathPlanRunner(grid _grid, List<(int, float[], float[])> _xBotID_From_To, int _xbotSize)
    {
        ...
        stopwatche.Stop();
        List<(int, List<(float, float)>)> output = unit_CM_To_M(_output); // Convert back to M
        return output;
    }

    public List<(int, int[], int[])> unit_M_To_CM(List<(int, float[], float[])> _xBotID_From_To)
    {
    ...
    public List<(int, List<(float, float)>)> unit_CM_To_M(List<(int, List<(int, int)>)> output)
    {
        List<(int, List<(float, float)>)> xBotID_From_To = new();
        foreach (var botTuple in output)
        {
            int botID = botTuple.Item1;
            List<(float, float)> path = botTuple.Item2.Select(x => ((float)x.Item1 / 100, (float)x.Item2 / 100)).ToList();
